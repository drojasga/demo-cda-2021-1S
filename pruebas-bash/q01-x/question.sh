...
cat data.csv | sed 's/^\([0-9]\)\//0\1\//g' | sed 's/\/\([0-9]\)\//\/0\1\//g' | sed 's/\/\([0-9][0-9]\);/\/20\1;/g' |sed -e 's/\(.*\)/\U\1/' | sed 's/;N;/;\\N;/g' | sed 's/;;/;\\N;/g' | sed 's/,/./g' |sed 's/;/,/g'| sed 's/\([0-9][0-9]\)\/\([0-9][0-9]\)\/\([0-9][0-9][0-9][0-9]\),/\3-\2-\1,/g' | sed 's/,\r$/,\\N\r/g' | sed 's/,n\r$/,\\N\r/g' |sed 's/,N\r$/,\\N\r/g'