...
cat -e data.csv | tr '/' '-' | sed 's/\-\([0-9][0-9]\)\;/\-20\1\;/;' | sed 's/\-\([0-9]\)\-/\-0\1\-/' | sed 's/^\([0-9]\)\-/0\1\-/' | sed 's/\([0-9]*\)\-\([0-9]*\)\-\([0-9]*[0-9]*\)/\3-\2-\1/' | tr ',' '.' | tr ';' ',' | sed 's/,,/,\\N,/' | sed 's/n/N/' | sed 's/,N/,\\N/'| tr [:lower:] [:upper:] | sed 's/,^$/,\\N/g'